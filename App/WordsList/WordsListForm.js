...
const WordsListForm = ({ wordListId, hdlModalClose }) => {
    ...
    useEffect(() => {
        ...
        const selectedWordIds = new Set(listData.words.map(word => word.id));

        // Filter available words to exclude those already in the list
        const availWords = wordsList.data.filter(word => !selectedWordIds.has(word.id));
    ...
    const handleTransferWord = (wordObj) => {
        ...
        if (Boolean(found)) {
            ...
            dispatch(actions.targetWordsList.create.itemSet({ words: newList }));
            ...
            dispatch(
                actions.targetWordsList.create.itemSet({
                    words: [...targetWordsList.item.words, {
                        ...wordObj,
    ...
    const filteredAvailableWords = availableWords.filter((entry) =>
        ...
        if (!groups[firstLetter]) {
            groups[firstLetter] = [];
                ...
                groups[firstLetter].push({
                    ...entry,
                    displayIpa: entry.ipa.split(' ').map(code => getIpaSymbol(code)).join(' ')
                });
        return groups;
    }, {});
            ...
            filteredAvailableWords.forEach(entry => {
                const phonemes = entry.ipa.split(' ').map(code => getIpaSymbol(code));
                ...
                phonemes.forEach((phoneme, index) => {
                    ...
                    if (!phonemeGroups[phoneme][position].find(item => item.word === entry.word)) {
            ...
            if (!selectedPhoneme) {
                return Object.entries(phonemeGroups)
                    .filter(([_, positions]) => 
                        Object.values(positions).some(words => words.length > 0)
                    )
            ...
            if (!selectedPosition) {
                const positions = phonemeGroups[selectedPhoneme] || { initial: [], medial: [], final: [] };
            ...
            const phonemeGroup = phonemeGroups[selectedPhoneme] || { initial: [], medial: [], final: [] };
    ...
    const hdlFieldChange = (field) => (event) => {
        dispatch(actions.targetWordsList.create.itemSet({
    ...
    const renderPhonemeContent = () => {
        ...
        if (!selectedPhoneme) {
            return (
                <Box sx={{ mb: 3 }}>
                    <Typography variant="h6" sx={{ mb: 2 }}>Select a Phoneme</Typography>
                        ...
                        {Object.entries(content).map(([phoneme, positions]) => (
                            ...
                            >
                                ...
                                <Typography sx={styles.phonemeSubtitle}>
                                    {Object.entries(content[phoneme]).length} positions
                                </Typography>
                            </Paper>
                        ))}
                    </Box>
                    ...
                    {!selectedPosition ? (
                        <>
                            ...
                            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                                {Object.entries(content).map(([key, items]) => {
                                    const position = key.split(' (')[1].replace(')', '');
                                    return (
                                        <Button
                                            key={position}
                                            variant="outlined"
                        ...
                        <>
                            ...
                            <Box sx={styles.wordGrid}>
                                {Object.values(content)[0].map((entry, index) => (
                                    <Paper
                                        key={`${entry.word}-${index}`}
                                        elevation={0}
                                        sx={styles.wordPaper}
                                        onClick={() => handleTransferWord({
                                            ...entry,
                                            noOfInstances: 3
                                        })}
                                    >
                                        <Word
                                            word={entry.word}
                                            ipa={entry.ipaSymbol || entry.displayIpa || entry.ipa}
                                        />
                                    </Paper>
                                ))}
                            </Box>
    ...
    const hdlDeleteList = async () => {
        setLoading(true);
        try {
            await dispatch(actions.targetWordsList.create.deleteTargetList(wordListId));
    ...
    const handleSaveList = async () => {
        ...
        // Validate name
        if (!targetWordsList.item.name || targetWordsList.item.name.trim() === '') {
            setValidationError('Please enter a list name');
            return;
        }

        // Validate words
        if (!targetWordsList.item.words || targetWordsList.item.words.length === 0) {
        ...
        try {
            if (!wordListId) {
                await dispatch(actions.targetWordsList.create.postTargetList(targetWordsList.item));
            } else {
                await dispatch(actions.targetWordsList.create.putTargetList(wordListId, targetWordsList.item));
            }
            enqueueSnackbar(`List ${wordListId === 0 ? 'created' : 'updated'} successfully`, {
                variant: "info"
            });
            hdlModalClose();
        } catch (error) {
    ...
    return (
        <Box sx={styles.formContainer}>
            <Box sx={styles.formContent}>
                <Box sx={styles.headerSection}>
                    ...
                    <TextField
                        ...
                        onChange={hdlFieldChange("name")}
                            ...
                            sx={styles.nameField}
                            required
                            error={validationError && !targetWordsList.item.name}
                            helperText={validationError && !targetWordsList.item.name ? 'Name is required' : ''}
                        />
                    <TextField
                        label="Description"
                        variant="outlined"
                ...
                <Box sx={styles.mainContent}>
                    <Paper elevation={0} sx={styles.sidePanel}>
                        ...
                        <Box sx={styles.panelContent}>
                            <List>
                                {targetWordsList.item.words.map((entry, index) => (
                                    <Paper 
                                        key={`${entry.word}-${index}`}
                                        elevation={0}
                                        sx={styles.wordCard}
                                ...
                                <IconButton
                                    aria-label="remove"
                                    onClick={() => handleTransferWord(entry)}
                                                    size="small"
                                ...
                                <ListItemText
                                                primary={
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                        ...
                                                        {entry.selectedPhoneme && entry.selectedPosition && (
                                                            <Chip 
                    ...
                    <Paper elevation={0} sx={styles.sidePanel}>
                        <Box sx={styles.panelHeader}>
                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                <Typography variant="h6">Available Words</Typography>
                                <Tooltip title="Select words by their phoneme positions. Words are grouped by where the phoneme appears in the word (initial, medial, or final position).">
                                    <InfoIcon sx={{ ml: 1, color: 'info.main', fontSize: '20px' }} />
                                </Tooltip>
                            </Box>
                        ...
                        <Box sx={styles.panelContent}>
                            {viewMode === 'phoneme' ? renderPhonemeContent() : (
                                Object.keys(groupedContent).map((key) => (
                                    <Box key={key} sx={{ mb: 3 }}>
                                        ...
                                        <Box sx={styles.alphabeticalGrid}>
                                            {groupedContent[key].map((entry, index) => (
                                                <Paper
                                                    key={`${entry.word}-${index}`}
                                                    elevation={0}
                                                    sx={styles.wordItem}
                ...
                <Box sx={{...styles.footer, justifyContent: wordListId > 0 ? 'space-between' : 'flex-end'}}>
                    {validationError && (
